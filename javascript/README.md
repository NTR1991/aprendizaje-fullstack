# 🟨 JavaScript Learning Path

![JavaScript](https://img.shields.io/badge/JavaScript-ES6%2B-F7DF1E?style=flat&logo=javascript&logoColor=black)
![Status](https://img.shields.io/badge/Status-In%20Progress-yellow)
![Blocks](https://img.shields.io/badge/Blocks-6-blue)

## 📋 Description

This repository contains my complete JavaScript learning journey following the W3Schools JavaScript tutorial. Each topic is practiced with real‑world business scenarios and includes exercises at normal, intermediate, and difficult levels, plus a final test.

All exercises are developed following professional coding standards with proper documentation and folder structures.

---

## 📂 Repository Structure

````
javascript/
│
├── 📁 01-Fundamentals/ ✅ (Bloque 1 Completado)
│   ├── 📁 01-introduction/ ✅
│   ├── 📁 02-variables/ ✅
│   ├── 📁 03-operators/ ✅
│   ├── 📁 04-conditionalStatements/ ✅
│   ├── 📁 05-functions/ ✅
│   ├── 📁 06-arrays/ ✅
│   └── 📄 README.md ✅
│
├── 📁 02-ControlFlow/ ✅ (Bloque 2 Completado)
│ ├── 📁 07-objects/ ✅
│ ├── 📁 08-arrays-methods/ ✅
│ ├── 📁 09-json/ ✅
│ ├── 📁 10-destructuring/ ✅
│ ├── 📁 11-spread-rest/ ✅
│ ├── 📁 12-dates/ ✅
│ └── 📄 README.md ✅
│
├── 📁 03-FunctionsArrays/ ✅ (Bloque 3 Completado)
│ ├── 📁 13-higher-order-functions/ ✅
│ ├── 📁 14-callbacks/ ✅
│ ├── 📁 15-closures/ ✅
│ ├── 📁 16-recursion/ ✅
│ ├── 📁 17-advanced-arrays/ ✅
│ ├── 📁 18-array-chaining/ ✅
│ ├── 📁 19-array-from/ ✅
│ ├── 📁 20-array-of/ ✅
│ └── 📄 README.md ✅
│
├── 📁 04-DOMEvents/ ⏳ (Bloque 4 Pendiente)
│   ├── 📁 21-dom-selectors/ ⏳
│   ├── 📁 22-dom-manipulation/ ⏳
│   ├── 📁 23-dom-styles/ ⏳
│   ├── 📁 24-dom-traversal/ ⏳
│   ├── 📁 25-event-listeners/ ⏳
│   ├── 📁 26-event-object/ ⏳
│   ├── 📁 27-forms-events/ ⏳
│   ├── 📁 28-localStorage/ ⏳
│   └── 📄 README.md ⏳
│
├── 📁 05-AsyncAPI/ ⏳ (Bloque 5 Pendiente)
│   ├── 📁 29-callbacks/ ⏳
│   ├── 📁 30-promises/ ⏳
│   ├── 📁 31-async-await/ ⏳
│   ├── 📁 32-fetch-api/ ⏳
│   ├── 📁 33-json-server/ ⏳
│   ├── 📁 34-rest-api/ ⏳
│   ├── 📁 35-error-handling/ ⏳
│   ├── 📁 36-web-storage/ ⏳
│   ├── 📁 37-geolocation/ ⏳
│   ├── 📁 38-notifications/ ⏳
│   └── 📄 README.md ⏳
│
├── 📁 06-FinalProject/ ⏳ (Bloque 6 Pendiente)
│   ├── 📁 39-project-setup/ ⏳
│   ├── 📁 40-project-login/ ⏳
│   ├── 📁 41-project-crud/ ⏳
│   ├── 📁 42-project-api/ ⏳
│   ├── 📁 43-project-testing/ ⏳
│   ├── 📁 44-project-deployment/ ⏳
│   └── 📄 README.md ⏳
│
└── 📄 README.md ✅ (Principal actualizado)
````




---

## 🎯 Learning Path

| Block | Topics | Status | Project |
|-------|--------|--------|---------|
| 01 | Fundamentals (6 topics) | ✅ Completed | To-Do List (Console) |
| 02 | Control Flow & Data Structures (6 topics) | ✅ Completed | Event Manager (Console) |
| 03 | Functions & Advanced Arrays (8 topics) | ✅ Completed | Product Catalog |
| 04 | DOM & Events (8 topics) | ⏳ Pending | Interactive Notes App |
| 05 | Async & Web APIs (10 topics) | ⏳ Pending | Weather Dashboard |
| 06 | Final Project (14 topics) | ⏳ Pending | Full Stack Application |

---

## 🏆 Projects

| Block | Project | Status | Repository |
|-------|---------|--------|------------|
| 1 | To-Do List (Console) | ✅ Completed | [View Project](https://github.com/NTR1991/aprendizaje-fullstack/tree/main/my-projects/16-To-Do%20List) |
| 2 | Event Manager (Console) | ✅ Completed | [View Project](https://github.com/NTR1991/aprendizaje-fullstack/tree/main/my-projects/18-js-event-manager) |
| 3 | Product Catalog (Console) | ✅ Completed | [View Project](https://github.com/NTR1991/aprendizaje-fullstack/tree/main/my-projects/19-js-product-catalog) |
| 4 | Interactive Notes App | ⏳ Pending | - |
| 5 | Weather Dashboard | ⏳ Pending | - |
| 6 | Full Stack Application | ⏳ Pending | - |

---

## 📊 Progress

**Completed:** 20 of 52 topics  
**Percentage:** 38% completed  
**Current:** Block 4 – Topic 21 (DOM Selectors) – Pending

### Block 1 – Fundamentals (✅ Completed)
- ✅ 01-introduction
- ✅ 02-variables
- ✅ 03-operators
- ✅ 04-conditionalStatements
- ✅ 05-functions
- ✅ 06-arrays

### Block 2 – Control Flow & Data Structures (✅ Completed)
- ✅ 07-objects
- ✅ 08-arrays-methods
- ✅ 09-json
- ✅ 10-destructuring
- ✅ 11-spread-rest
- ✅ 12-dates

### Block 3 – Functions & Advanced Arrays (✅ Completed)
- ✅ 13-higher-order-functions
- ✅ 14-callbacks
- ✅ 15-closures
- ✅ 16-recursion
- ✅ 17-advanced-arrays
- ✅ 18-array-chaining
- ✅ 19-array-from
- ✅ 20-array-of

### Block 4 – DOM & Events (⏳ Pending)
- ⏳ 21-dom-selectors
- ⏳ 22-dom-manipulation
- ⏳ 23-dom-styles
- ⏳ 24-dom-traversal
- ⏳ 25-event-listeners
- ⏳ 26-event-object
- ⏳ 27-forms-events
- ⏳ 28-localStorage

---

## 📋 Learning Methodology

Each topic follows the same structure:

1. 📚 **Detailed explanation** with real‑world examples
2. 🟢 **Normal exercises** – Basic scenarios
3. 🟡 **Intermediate exercises** – Combined concepts
4. 🔴 **Difficult exercises** – Complex real‑world problems
5. 🧪 **Test** – Knowledge verification
6. 📝 **README.md** – Documentation with exercise statements (no code)

---

## 🛠️ Technologies Used

- **JavaScript (ES6+)** – Core language
- **HTML5** – Semantic structure
- **CSS3** – Professional styling
- **Git** – Version control
- **GitHub** – Repository hosting

---

## 📌 Quick Links

- [Block 1: Fundamentals](01-Fundamentals/README.md)
- [Block 2: Control Flow](02-ControlFlow/README.md)
- [Block 3: Functions & Advanced Arrays](03-FunctionsArrays/README.md)
- [Block 4: DOM & Events](04-DOMEvents/README.md)
- [Block 5: Async & Web APIs](05-AsyncAPI/README.md)
- [Block 6: Final Project](06-FinalProject/README.md)

---

## 👤 Author

*NTR1991 – Full Stack Developer in training | FP DAW Student*

## 📅 Date

July 2026
